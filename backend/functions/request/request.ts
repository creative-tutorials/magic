...
dotenv.config();
import { RequestsRecord, getXataClient } from "../../database/xata.js";
...
async function createRequest(
  ...
  try {
    ...
    if (!record) throw new Error("Failed to submit request");
 ...
 */
async function checkDuplicate(name: string, email: string): Promise<boolean> {
  try {
 ...
 */
async function getRequestsData(): Promise<
  RecordArray<
    SelectedPick<
      RequestsRecord,
      ("appname" | "description" | "url" | "email" | "username")[]
    >
  >
> {
  try {
 ...
 */
async function declineRequest(id: string): Promise<string> {
  try {
    const request = await xata.db.requests.delete(id);

    if (!request) throw new Error("Failed to decline request");
